...
function LoadingState({ label = "Loading your workspace…" }: { label?: string }) {
  return <div className="flex items-center gap-2 py-12 text-sm text-muted-foreground"><LoaderCircle size={16} className="animate-spin" /> {label}</div>;
}

function OfflineState({ onRetry }: { onRetry: () => void }) {
  return <Card className="flex flex-col items-start gap-4 p-6"><Status tone="amber">Backend unavailable</Status><p className="max-w-xl text-sm leading-relaxed text-muted-foreground">The dashboard is ready for the Chusky API, but the local service did not respond. Start the backend and retry.</p><Button secondary onClick={onRetry}><RefreshCw size={14} /> Retry</Button></Card>;
}
...
function ThreadRows({ threads }: { threads: Thread[] }) {
  ...
  return <div>{threads.map((thread) => <div key={thread.id} className="flex items-center justify-between gap-4 border-b border-foreground/10 px-5 py-4 last:border-0"><div className="min-w-0"><p className="truncate text-sm font-medium">{String(thread.metadata.title || thread.metadata.prompt || "Untitled conversation")}</p><p className="mt-1 text-xs text-muted-foreground">Updated {formatDate(thread.updatedAt)}</p></div><ArrowUpRight size={14} className="shrink-0 text-muted-foreground" /></div>)}</div>;
}
...
export function BackendDashboardPage() {
  ...
  useEffect(() => { void load(); }, []);
  return <><PageHeading eyebrow="Workspace overview" title="Your agent at a glance." description="Live activity and usage from your authenticated Chusky workspace." action={<Button onClick={() => { window.location.href = "/app/chat"; }}>New conversation</Button>} />{offline ? <OfflineState onRetry={() => void load()} /> : !data ? <LoadingState /> : <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Messages" value={data.usage.messages.toLocaleString()} detail="This billing period" /><Metric label="Active runs" value={data.usage.runs.active.toLocaleString()} detail={`${data.usage.runs.count.toLocaleString()} total runs`} /><Metric label="Tasks" value={data.usage.tasks.count.toLocaleString()} detail="Tracked by Chusky" /><Metric label="File storage" value={`${data.usage.files.available.toLocaleString()} bytes`} detail={`${data.usage.files.count} uploaded files`} /></div><Card className="mt-8"><div className="border-b border-foreground/10 p-5"><h2 className="font-display text-2xl">Recent conversations</h2><p className="mt-1 text-xs text-muted-foreground">Synced from the Chusky Developer API</p></div><ThreadRows threads={data.threads.data.slice(0, 5)} /></Card></>}</>;
}

function Metric({ label, value, detail }: { label: string; value: string; detail: string }) { return <Card className="p-5"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{label}</p><p className="mt-5 font-display text-4xl">{value}</p><p className="mt-1 text-xs text-muted-foreground">{detail}</p></Card>; }
...
export function BackendTasksPage() {
  ...
  useEffect(() => { void load(); }, []);
  return <><PageHeading eyebrow="Execution" title="Tasks" description="Long-running work managed by Chusky." />{offline ? <OfflineState onRetry={() => void load()} /> : !page ? <LoadingState /> : <Card>{page.data.length ? page.data.map((task) => <div key={task.id} className="border-b border-foreground/10 px-5 py-4 last:border-0"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-medium">{task.title}</p><p className="mt-1 text-xs text-muted-foreground">Updated {formatDate(task.updatedAt)}</p></div><Status tone={task.status === "failed" ? "amber" : task.status === "completed" ? "green" : "gray"}>{task.status}</Status></div>{task.checkpoint && <p className="mt-3 text-xs text-muted-foreground">{task.checkpoint}</p>}</div>) : <p className="p-5 text-sm text-muted-foreground">No tasks yet.</p>}</Card>}</>;
}
