...
export function AutonomyPage() {
  ...
  return <div>
    ...
    {loading && !snapshot ? <Card className="p-5 text-[11px] text-muted-foreground">Loading durable autonomy state…</Card> : snapshot && <>
      ...
      <Card><div className="border-b border-foreground/10 px-4 py-3"><h2 className="text-xs font-medium">Next actions</h2><p className="mt-0.5 text-[10px] text-muted-foreground">Only durable items and verified checkpoints appear here.</p></div><div className="divide-y divide-foreground/10">{snapshot.queue.length ? snapshot.queue.slice(0, 40).map((item) => <div key={`${item.kind}:${item.id}`} className="flex items-start justify-between gap-3 px-4 py-3"><div className="min-w-0"><p className="truncate text-[11px] font-medium">{item.title}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{item.nextAction || item.source} · next check {date(item.nextCheckAt)}</p>{item.blockedReason && <p className="mt-1 text-[10px] text-amber-700">{item.blockedReason}</p>}</div><Status tone={item.blockedReason ? "amber" : "green"}>{item.status}</Status></div>) : <p className="px-4 py-8 text-center text-[11px] text-muted-foreground">Nothing is waiting for attention.</p>}</div></Card>
    </>}
  </div>;
}
