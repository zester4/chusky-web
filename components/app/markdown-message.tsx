  ...
  return <figure className="my-2 max-w-full overflow-hidden rounded-md border border-foreground/10 bg-background p-2 sm:p-2.5">
    ...
    <div className="h-44 min-w-0 w-full sm:h-48">
      <ResponsiveContainer width="100%" height="100%">
        {spec.type === "pie" ? <PieChart><Tooltip contentStyle={{ borderRadius: 6, borderColor: "#e5e7eb", fontSize: 11 }} /><Legend wrapperStyle={{ fontSize: 10 }} /><Pie data={data} dataKey={spec.valueKey || series[0]} nameKey={xKey} cx="50%" cy="50%" outerRadius="70%" paddingAngle={2}>{data.map((_, index) => <Cell key={`cell-${index}`} fill={chartColors[index % chartColors.length]} />)}</Pie></PieChart>
          : spec.type === "bar" ? <BarChart {...common}><CartesianGrid vertical={false} stroke="#e5e7eb" /><XAxis dataKey={xKey} tick={{ fontSize: 9 }} tickLine={false} axisLine={false} /><YAxis tick={{ fontSize: 9 }} tickLine={false} axisLine={false} /><Tooltip contentStyle={{ borderRadius: 6, borderColor: "#e5e7eb", fontSize: 11 }} /><Legend wrapperStyle={{ fontSize: 10 }} />{series.map((key, index) => <Bar key={key} dataKey={key} fill={chartColors[index % chartColors.length]} radius={[3, 3, 0, 0]} maxBarSize={28} />)}</BarChart>
          : spec.type === "area" ? <AreaChart {...common}><defs>{series.map((key, index) => <linearGradient key={key} id={`${idSafe(key)}-gradient`} x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={chartColors[index % chartColors.length]} stopOpacity={0.28} /><stop offset="95%" stopColor={chartColors[index % chartColors.length]} stopOpacity={0.02} /></linearGradient>)}</defs><CartesianGrid vertical={false} stroke="#e5e7eb" /><XAxis dataKey={xKey} tick={{ fontSize: 9 }} tickLine={false} axisLine={false} /><YAxis tick={{ fontSize: 9 }} tickLine={false} axisLine={false} /><Tooltip contentStyle={{ borderRadius: 6, borderColor: "#e5e7eb", fontSize: 11 }} /><Legend wrapperStyle={{ fontSize: 10 }} />{series.map((key, index) => <Area key={key} type="monotone" dataKey={key} stroke={chartColors[index % chartColors.length]} fill={`url(#${idSafe(key)}-gradient)`} strokeWidth={1.5} />)}</AreaChart>
          : <LineChart {...common}><CartesianGrid vertical={false} stroke="#e5e7eb" /><XAxis dataKey={xKey} tick={{ fontSize: 9 }} tickLine={false} axisLine={false} /><YAxis tick={{ fontSize: 9 }} tickLine={false} axisLine={false} /><Tooltip contentStyle={{ borderRadius: 6, borderColor: "#e5e7eb", fontSize: 11 }} /><Legend wrapperStyle={{ fontSize: 10 }} />{series.map((key, index) => <Line key={key} type="monotone" dataKey={key} stroke={chartColors[index % chartColors.length]} strokeWidth={1.5} dot={{ r: 2 }} />)}</LineChart>}
      </ResponsiveContainer>
    </div>
  </figure>;
}
...
const components: Components = {
  ...
  img: ({ alt, ...props }) => <img alt={alt || ""} loading="lazy" className="my-2 max-h-72 max-w-full rounded-md border border-foreground/10 object-contain" {...props} />,
