...
import { Bell, LogOut, Menu, PanelLeftClose, PanelLeftOpen, Plus, Search, X } from "lucide-react";
...
const primary = [
  ...
  ["Approvals", "/app/approvals", "✓"], ["Calls", "/app/calls", "⌕"], ["Meetings", "/app/meetings", "◷"], ["Connected apps", "/app/apps", "⊞"], ["Channels", "/app/channels", "◉"], ["Capabilities", "/app/capabilities", "✧"], ["Workers", "/app/workers", "◈"], ["Tasks", "/app/tasks", "▣"], ["Operations", "/app/operations", "◉"], ["Delivery", "/app/delivery", "↗"],
];
const work = [["Organizations", "/app/organizations", "◫"], ["MCP connections", "/app/mcp", "⌁"], ["Workflow Composer", "/app/composer", "⌘"], ["Reminders", "/app/reminders", "◷"], ["Recurring jobs", "/app/jobs", "↻"], ["Skills", "/app/skills", "▱"], ["Memory", "/app/memory", "◇"], ["Scratchpad", "/app/scratchpad", "✎"], ["Triggers", "/app/triggers", "⌁"], ["Workspace", "/app/workspace", "▤"]];
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <AppShellContext.Provider value={{ setChatHeader }}><div className="app-shell min-h-svh text-foreground" style={{ backgroundColor: branding?.backgroundColor || "#f7f7f4" }}>
    <aside className={cn("fixed inset-y-0 left-0 z-40 hidden w-[232px] flex-col overflow-hidden border-r border-foreground/10 bg-background transition-[transform,width] duration-200 lg:flex", collapsed && "lg:w-[68px]")}>
      ...
      <div className={cn("border-t border-foreground/10 p-2.5", collapsed && "lg:p-2")}><div className={cn("flex min-w-0 items-center gap-2", collapsed && "lg:justify-center")}><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-foreground text-[10px] text-background">{accountName.slice(0, 2).toUpperCase()}</span><span className={cn("min-w-0 flex-1 truncate text-[11px] font-medium", collapsed && "lg:hidden")}>{accountName}</span><button onClick={signOut} className={cn("inline-flex min-h-9 shrink-0 items-center justify-center gap-1.5 rounded-md px-2 text-[10px] text-muted-foreground hover:bg-foreground/5 hover:text-foreground", collapsed && "lg:min-w-9 lg:px-0")} aria-label="Sign out" title="Sign out"><span className={collapsed ? "lg:hidden" : undefined}>Sign out</span><LogOut size={13} className={collapsed ? "hidden lg:block" : "hidden"}/></button></div></div>
    </aside>
    <Drawer open={open} onOpenChange={setOpen} direction="left"><DrawerContent className="h-full max-h-none w-[min(20rem,calc(100vw-1rem))] p-0"><div className="flex min-h-0 flex-1 flex-col overflow-hidden"><div className="flex h-14 items-center justify-between border-b border-foreground/10 px-4"><Link href="/app" className="flex items-center gap-2 font-display text-xl tracking-tight">{branding?.logoUrl ? <img src={branding.logoUrl} alt="" className="h-6 w-6 object-contain" /> : brandName}<span className="ml-1 align-top font-mono text-[9px] text-muted-foreground">TM</span></Link><DrawerClose asChild><button className="flex min-h-9 min-w-9 items-center justify-center text-muted-foreground hover:text-foreground" aria-label="Close menu"><X size={18}/></button></DrawerClose></div><div className="p-3"><button type="button" onClick={startNewConversation} className="flex min-h-9 w-full items-center gap-2 border border-foreground/15 bg-background px-2.5 py-2 text-xs hover:border-foreground/40"><Plus size={14}/> New conversation</button></div><nav className="min-h-0 flex-1 overflow-y-auto px-2"><p className="px-2 pb-1.5 pt-1 font-mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground">Workspace</p>{nav(primary)}<p className="px-2 pb-1.5 pt-5 font-mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground">Automate</p>{nav(work)}<p className="px-2 pb-1.5 pt-5 font-mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground">Account</p>{nav([["Developer API", "/app/developer-api", "⌘"], ["Devices", "/app/devices", "⌁"], ["Settings", "/app/settings", "⚙"]])}</nav><div className="m-3 border border-foreground/10 bg-foreground/[0.03] p-2"><div className="flex items-center gap-2 text-[10px] font-medium"><span className={cn("h-2 w-2 shrink-0 rounded-full", health?.ok ? "bg-emerald-500" : health ? "bg-amber-400" : "bg-foreground/25")}/><span>{health ? health.ok ? "All systems operational" : "System needs attention" : "Checking system status"}</span></div></div><div className="border-t border-foreground/10 p-3"><div className="flex min-w-0 items-center gap-2"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-foreground text-[10px] text-background">{accountName.slice(0, 2).toUpperCase()}</span><span className="min-w-0 flex-1 truncate text-[11px] font-medium">{accountName}</span><button onClick={signOut} className="inline-flex min-h-9 shrink-0 items-center justify-center rounded-md px-2 text-[10px] text-muted-foreground hover:bg-foreground/5 hover:text-foreground" aria-label="Sign out">Sign out</button></div></div></div></DrawerContent></Drawer>
    <div className={cn("min-w-0 transition-[padding] duration-200", collapsed ? "lg:pl-[68px]" : "lg:pl-[232px]")}><header className="sticky top-0 z-20 flex h-12 items-center justify-between border-b border-foreground/10 bg-background/90 px-2.5 backdrop-blur-xl sm:h-14 sm:px-4 lg:px-6"><div className="flex min-w-0 items-center gap-2.5"><button onClick={() => setOpen(true)} className="flex min-h-9 min-w-9 items-center justify-center lg:hidden" aria-label="Open menu"><Menu size={18}/></button>{pathname === "/app/chat" ? <div className="flex min-w-0 items-center gap-2"><h1 className="max-w-[calc(100vw-9rem)] truncate text-[11px] font-medium sm:max-w-[24rem]">{chatHeader?.title || "Chat"}</h1><span className={chatHeader?.status === "ready" ? "shrink-0 rounded-full bg-emerald-100 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider text-emerald-800" : "shrink-0 rounded-full bg-amber-100 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider text-amber-800"}>{chatHeader?.status === "ready" ? "Live" : chatHeader?.status === "offline" ? "Offline" : "Connecting"}</span></div> : <button type="button" onClick={() => router.push("/app/conversations")} className="hidden items-center gap-2 text-[10px] text-muted-foreground hover:text-foreground sm:flex" aria-label="Search conversations"><Search size={12}/> Search conversations <kbd className="border border-foreground/15 px-1.5 py-0.5 font-mono text-[9px]">⌘K</kbd></button>}</div><button type="button" onClick={() => router.push("/app/approvals")} className="flex min-h-9 min-w-9 items-center justify-center text-muted-foreground hover:text-foreground" aria-label="Open approvals and activity"><Bell size={16}/></button></header><main className="mx-auto max-w-[1400px] min-w-0 px-2.5 py-3.5 sm:px-4 sm:py-6 lg:px-7 lg:py-8">{children}</main></div>
  </div></AppShellContext.Provider>;
}

export function PageHeading({ eyebrow, title, description, action }: { eyebrow: string; title: string; description?: string; action?: ReactNode }) { return <div className="mb-4 flex min-w-0 flex-col justify-between gap-2 border-b border-foreground/10 pb-3.5 sm:mb-5 sm:flex-row sm:items-end sm:gap-3 sm:pb-4"><div className="min-w-0"><p className="mb-1 font-mono text-[8px] uppercase tracking-[0.2em] text-muted-foreground">{eyebrow}</p><h1 className="font-display text-[1.4rem] leading-none tracking-tight sm:text-[1.75rem]">{title}</h1>{description && <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-muted-foreground">{description}</p>}</div>{action && <div className="flex shrink-0">{action}</div>}</div>; }
export function Button({ children, secondary = false, onClick, disabled }: { children: ReactNode; secondary?: boolean; onClick?: () => void; disabled?: boolean }) { return <button onClick={onClick} disabled={disabled} className={cn("inline-flex w-fit max-w-full self-start min-h-7 shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-lg px-2.5 text-[10px] font-medium transition-[color,background-color,border-color,transform] hover:-translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50", secondary ? "border border-foreground/12 bg-background shadow-sm hover:border-foreground/30 hover:bg-foreground/[0.03]" : "bg-foreground text-background shadow-sm hover:bg-foreground/85")}>{children}</button>; }
export function Card({ children, className }: { children: ReactNode; className?: string }) { return <div className={cn("min-w-0 overflow-hidden rounded-xl border border-foreground/10 bg-background shadow-[0_1px_2px_rgb(0_0_0/0.03)]", className)}>{children}</div>; }
export function Status({ children, tone = "green" }: { children: ReactNode; tone?: "green" | "amber" | "gray" }) { return <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-medium", tone === "green" ? "bg-emerald-500/10 text-emerald-700" : tone === "amber" ? "bg-amber-400/15 text-amber-700" : "bg-foreground/[0.06] text-muted-foreground")}><span className={cn("h-1.5 w-1.5 rounded-full", tone === "green" ? "bg-emerald-500" : tone === "amber" ? "bg-amber-400" : "bg-foreground/25")}/>{children}</span>; }
