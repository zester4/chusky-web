...
const primary = [
  ...
  ["Approvals", "/app/approvals", "✓"], ["Calls", "/app/calls", "⌕"], ["Autonomy", "/app/autonomy", "◎"], ["Meetings", "/app/meetings", "◷"], ["Connected apps", "/app/apps", "⊞"], ["Channels", "/app/channels", "◉"], ["Capabilities", "/app/capabilities", "✧"], ["Workers", "/app/workers", "◈"], ["Tasks", "/app/tasks", "▣"], ["Missions", "/app/missions", "↻"], ["Operations", "/app/operations", "◉"], ["Delivery", "/app/delivery", "↗"],
];
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <AppShellContext.Provider value={{ setChatHeader }}><div className="app-shell min-h-svh text-foreground" style={{ backgroundColor: branding?.backgroundColor || "#f7f7f4" }}>
    ...
    <div className={cn("min-w-0 transition-[padding] duration-200", collapsed ? "lg:pl-[68px]" : "lg:pl-[232px]")}><header className="sticky top-0 z-20 flex h-11 items-center justify-between border-b border-foreground/10 bg-background/90 px-2.5 backdrop-blur-xl sm:h-14 sm:px-4 lg:px-6"><div className="flex min-w-0 items-center gap-2.5"><button onClick={() => setOpen(true)} className="flex min-h-9 min-w-9 items-center justify-center lg:hidden" aria-label="Open menu"><Menu size={17}/></button>{pathname === "/app/chat" ? <div className="flex min-w-0 items-center gap-2"><h1 className="max-w-[calc(100vw-9rem)] truncate text-[11px] font-medium sm:max-w-[24rem]">{chatHeader?.title || "Chat"}</h1><span className={chatHeader?.status === "ready" ? "shrink-0 rounded-full bg-emerald-100 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider text-emerald-800" : "shrink-0 rounded-full bg-amber-100 px-1.5 py-0.5 font-mono text-[8px] uppercase tracking-wider text-amber-800"}>{chatHeader?.status === "ready" ? "Live" : chatHeader?.status === "offline" ? "Offline" : "Connecting"}</span></div> : <button type="button" onClick={() => router.push("/app/conversations")} className="hidden items-center gap-2 text-[10px] text-muted-foreground hover:text-foreground sm:flex" aria-label="Search conversations"><Search size={12}/> Search conversations <kbd className="border border-foreground/15 px-1.5 py-0.5 font-mono text-[9px]">⌘K</kbd></button>}</div><button type="button" onClick={() => router.push("/app/approvals")} className="flex min-h-9 min-w-9 items-center justify-center text-muted-foreground hover:text-foreground" aria-label="Open approvals and activity"><Bell size={15}/></button></header><main className="mx-auto max-w-[1400px] min-w-0 px-2.5 py-2.5 sm:px-4 sm:py-6 lg:px-7 lg:py-8">{children}</main></div>
  </div></AppShellContext.Provider>;
}

export function PageHeading({ eyebrow, title, description, action }: { eyebrow: string; title: string; description?: string; action?: ReactNode }) { return <div className="app-page-heading relative mb-4 flex min-w-0 justify-between gap-3 border-b border-foreground/10 pb-3.5 sm:mb-5 sm:items-end sm:pb-4"><div className={cn("min-w-0", action && "pr-28 sm:pr-0")}><p className="mb-1 font-mono text-[8px] uppercase tracking-[0.2em] text-muted-foreground">{eyebrow}</p><h1 className="font-display text-[1.4rem] leading-none tracking-tight sm:text-[1.75rem]">{title}</h1>{description && <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-muted-foreground">{description}</p>}</div>{action && <div className="app-page-heading-actions absolute right-0 top-0 flex max-w-[8rem] shrink-0 flex-wrap justify-end gap-1.5 sm:static sm:max-w-none sm:justify-end">{action}</div>}</div>; }
export function Button({ children, secondary = false, onClick, disabled, className }: { children: ReactNode; secondary?: boolean; onClick?: () => void; disabled?: boolean; className?: string }) { return <button onClick={onClick} disabled={disabled} className={cn("app-button inline-flex w-fit max-w-full self-start min-h-9 shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-md px-2 text-[10px] font-medium transition-[color,background-color,border-color,transform] hover:-translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50 sm:min-h-7", secondary ? "border border-foreground/12 bg-background shadow-sm hover:border-foreground/30 hover:bg-foreground/[0.03]" : "bg-foreground text-background shadow-sm hover:bg-foreground/85", className)}>{children}</button>; }
...
export function Status({ children, tone = "green" }: { children: ReactNode; tone?: "green" | "amber" | "gray" }) { return <span className={cn("inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[9px] font-medium", tone === "green" ? "bg-emerald-500/10 text-emerald-700" : tone === "amber" ? "bg-amber-400/15 text-amber-700" : "bg-foreground/[0.06] text-muted-foreground")}><span className={cn("h-1.5 w-1.5 rounded-full", tone === "green" ? "bg-emerald-500" : tone === "amber" ? "bg-amber-400" : "bg-foreground/25")}/>{children}</span>; }
