...
export function Button({ children, secondary = false, onClick, disabled }: { children: ReactNode; secondary?: boolean; onClick?: () => void; disabled?: boolean }) { return <button onClick={onClick} disabled={disabled} className={cn("inline-flex w-fit max-w-full self-start min-h-8 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-full px-2.5 text-[11px] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30 disabled:cursor-not-allowed disabled:opacity-50", secondary ? "border border-foreground/10 bg-background hover:border-foreground/30" : "bg-foreground text-background hover:bg-foreground/85")}>{children}</button>; }
