...
const primary = [
  ...
  ["Approvals", "/app/approvals", "✓"], ["Calls", "/app/calls", "⌕"], ["Meetings", "/app/meetings", "◷"], ["Connected apps", "/app/apps", "⊞"], ["Channels", "/app/channels", "◉"], ["Capabilities", "/app/capabilities", "✧"], ["Workers", "/app/workers", "◈"], ["Tasks", "/app/tasks", "▣"], ["Missions", "/app/missions", "↻"], ["Operations", "/app/operations", "◉"], ["Delivery", "/app/delivery", "↗"],
];
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const nav = (items: string[][]) => items.map(([label, href, icon]) => (
    <Link key={href} href={href} onClick={() => setOpen(false)} aria-label={label} title={collapsed ? label : undefined} className={cn("flex min-h-10 min-w-0 items-center gap-2.5 rounded-lg py-2 text-xs transition-colors lg:min-h-0", collapsed ? "px-2.5 lg:justify-center lg:px-2" : "px-2.5", pathname === href ? "bg-foreground text-background" : "text-muted-foreground hover:bg-foreground/5 hover:text-foreground")}>
...
export function PageHeading({ eyebrow, title, description, action }: { eyebrow: string; title: string; description?: string; action?: ReactNode }) { return <div className="mb-4 flex min-w-0 flex-col justify-between gap-3 border-b border-foreground/10 pb-3.5 sm:mb-5 sm:flex-row sm:items-end sm:gap-3 sm:pb-4"><div className="min-w-0"><p className="mb-1 font-mono text-[8px] uppercase tracking-[0.2em] text-muted-foreground">{eyebrow}</p><h1 className="font-display text-[1.4rem] leading-none tracking-tight sm:text-[1.75rem]">{title}</h1>{description && <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-muted-foreground">{description}</p>}</div>{action && <div className="flex w-full shrink-0 flex-wrap gap-2 sm:w-auto sm:justify-end">{action}</div>}</div>; }
export function Button({ children, secondary = false, onClick, disabled, className }: { children: ReactNode; secondary?: boolean; onClick?: () => void; disabled?: boolean; className?: string }) { return <button onClick={onClick} disabled={disabled} className={cn("inline-flex w-fit max-w-full self-start min-h-9 shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-lg px-2.5 text-[10px] font-medium transition-[color,background-color,border-color,transform] hover:-translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50 sm:min-h-7", secondary ? "border border-foreground/12 bg-background shadow-sm hover:border-foreground/30 hover:bg-foreground/[0.03]" : "bg-foreground text-background shadow-sm hover:bg-foreground/85", className)}>{children}</button>; }
