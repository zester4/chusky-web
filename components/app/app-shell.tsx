...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <AppShellContext.Provider value={{ setChatHeader }}><div className="min-h-svh text-foreground" style={{ backgroundColor: branding?.backgroundColor || "#f7f7f4" }}>
...
export function PageHeading({ eyebrow, title, description, action }: { eyebrow: string; title: string; description?: string; action?: ReactNode }) { return <div className="mb-4 flex min-w-0 flex-col justify-between gap-2 border-b border-foreground/10 pb-3.5 sm:mb-5 sm:flex-row sm:items-end sm:gap-3 sm:pb-4"><div className="min-w-0"><p className="mb-1 font-mono text-[8px] uppercase tracking-[0.2em] text-muted-foreground">{eyebrow}</p><h1 className="font-display text-[1.5rem] leading-none tracking-tight sm:text-3xl">{title}</h1>{description && <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-muted-foreground">{description}</p>}</div>{action && <div className="flex shrink-0">{action}</div>}</div>; }
export function Button({ children, secondary = false, onClick, disabled }: { children: ReactNode; secondary?: boolean; onClick?: () => void; disabled?: boolean }) { return <button onClick={onClick} disabled={disabled} className={cn("inline-flex w-fit max-w-full self-start min-h-7 shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-md px-2 text-[10px] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/30 disabled:cursor-not-allowed disabled:opacity-50", secondary ? "border border-foreground/10 bg-background hover:border-foreground/30" : "bg-foreground text-background hover:bg-foreground/85")}>{children}</button>; }
