...
export function AccountDataPage({ kind }: { kind: PageKind }) {
  ...
  return <><PageHeading eyebrow={heading.eyebrow} title={heading.title} description={heading.description} action={<Button secondary onClick={() => void load()}><RefreshCw size={13} /> Refresh</Button>} />{offline ? <Offline retry={() => void load()} /> : !data ? <Card className="flex items-center gap-3 p-6 text-xs text-muted-foreground"><LoaderCircle size={15} className="animate-spin" /> Loading your saved data…</Card> : <Content kind={kind} data={data} decide={decide} busy={busy} />}</>;
}
...
function Content({ kind, data, decide, busy }: { kind: PageKind; data: AccountOverview; decide: (id: string, decision: "approve" | "deny") => Promise<void>; busy?: string }) {
  if (kind === "approvals") return <Card>{data.approvals.length ? data.approvals.map((item) => <div key={item.id} className="border-b border-foreground/10 p-4 last:border-0"><div className="flex flex-col gap-3 md:flex-row md:items-start"><ShieldCheck className="mt-1 text-amber-600" size={17} /><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><h2 className="text-xs font-medium">{item.toolSlug}</h2><Status tone="amber">Expires {date(item.expiresAt)}</Status></div><p className="mt-2 text-xs text-muted-foreground">{item.request}</p><p className="mt-2 font-mono text-[9px] text-muted-foreground">{item.id}{item.channelProvider ? ` · ${item.channelProvider}` : ""}</p></div><div className="flex gap-2"><Button secondary onClick={() => void decide(item.id, "deny")}><Trash2 size={12} /> Deny</Button><Button onClick={() => void decide(item.id, "approve")}><Check size={12} /> {busy === item.id ? "Working" : "Approve"}</Button></div></div></div>) : <Empty>No pending approvals. Chusky will show risky actions here before execution.</Empty>}</Card>;
...
function Row({ icon, title, detail, meta, status }: { icon: ReactNode; title: string; detail?: string; meta?: string; status?: string }) { return <div className="flex items-start gap-2.5 border-b border-foreground/10 px-4 py-3 last:border-0"><span className="flex h-7 w-7 shrink-0 items-center justify-center border border-foreground/10 text-muted-foreground">{icon}</span><div className="min-w-0 flex-1"><p className="break-words text-xs font-medium">{title}</p>{detail && <p className="mt-1 break-words text-[11px] text-muted-foreground">{detail}</p>}{meta && <p className="mt-1.5 text-[9px] text-muted-foreground">{meta}</p>}</div>{status && <Status tone={status === "failed" ? "amber" : status === "cancelled" ? "gray" : "green"}>{status}</Status>}</div>; }
