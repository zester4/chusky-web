...
import { Activity, Building2, CalendarClock, Check, Clock3, ExternalLink, Languages, LoaderCircle, Plus, RefreshCw, Save, ShieldCheck, Users, Video, X } from "lucide-react";
import { chuskyApi, type ConnectedAccount, type Meeting, type MeetingCapability, type MeetingContact, type MeetingNativeCapability, type MeetingProviderCapabilities, type MeetingRepresentativeProfile, type MeetingRoomPolicy, type MeetingWorkspace } from "@/lib/chusky-api";
...
function runtimeLabel(value?: Meeting["runtimeState"]) { return value ? value.replaceAll("_", " ") : "status pending"; }
function capabilityLabel(value: string) { return value.replaceAll(/([a-z])([A-Z])/g, "$1 $2").replaceAll("_", " "); }
function capabilityTone(value: string) { return value === "supported" ? "text-emerald-700" : value === "limited" ? "text-amber-700" : "text-muted-foreground"; }
function timelineLabel(value: string) { return value.replaceAll("_", " "); }

type MeetingContextDraft = { clientName: string; objective: string; clientContext: string; brief?: string; preparedAt?: number };

function MeetingContextPanel({ value, onChange, onPrepare, busy, compact = false }: { value: MeetingContextDraft; onChange: (patch: Partial<MeetingContextDraft>) => void; onPrepare: () => void; busy: boolean; compact?: boolean }) {
  return <div className={`border border-foreground/10 bg-foreground/[0.02] p-3 ${compact ? "mt-3" : "mt-4"}`}>
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-medium">Prepare private meeting context</p><p className="mt-1 text-[10px] leading-4 text-muted-foreground">Chusky searches only normal-sensitivity relationship and business facts. Nothing is shared until you attach the reviewed brief to the join.</p></div><ShieldCheck size={15} className="shrink-0 text-emerald-700"/></div>
    <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="block text-xs text-muted-foreground">Client or counterpart<input value={value.clientName} onChange={(event) => onChange({ clientName: event.target.value, brief: undefined, preparedAt: undefined })} placeholder="Acme or Jordan Lee" maxLength={120} className={inputClass}/></label><label className="block text-xs text-muted-foreground">Meeting objective<input value={value.objective} onChange={(event) => onChange({ objective: event.target.value, brief: undefined, preparedAt: undefined })} placeholder="Review the proposal and agree next steps" maxLength={1500} className={inputClass}/></label></div>
    <label className="mt-3 block text-xs text-muted-foreground">Owner-provided context <span className="font-normal">(optional)<textarea value={value.clientContext} onChange={(event) => onChange({ clientContext: event.target.value, brief: undefined, preparedAt: undefined })} placeholder="Facts or goals you want Chusky to consider" maxLength={4000} rows={2} className={textClass}/></span></label>
    <div className="mt-3 flex flex-wrap items-center gap-3"><Button secondary disabled={busy || !value.clientName.trim()} onClick={onPrepare}>{busy ? <LoaderCircle size={13} className="animate-spin"/> : <ShieldCheck size={13}/>} {value.brief ? "Refresh private brief" : "Prepare private brief"}</Button>{value.brief && <span className="text-[10px] text-emerald-700">Brief prepared for your review. Attach it when you join.</span>}</div>
    {value.brief && <div className="mt-3 border-l-2 border-emerald-600 pl-3"><p className="font-mono text-[9px] uppercase tracking-[0.15em] text-muted-foreground">Owner-only preview</p><p className="mt-1.5 max-h-48 overflow-y-auto whitespace-pre-wrap text-[11px] leading-5 text-foreground">{value.brief}</p></div>}
  </div>;
}
...
export function MeetingsPage() {
  ...
  const [meetingUrl, setMeetingUrl] = useState(""); const [selectedRoomId, setSelectedRoomId] = useState(""); const [joinMode, setJoinMode] = useState<Meeting["interactionMode"]>("addressed"); const [languageMode, setLanguageMode] = useState<NonNullable<Meeting["languageMode"]>>("english"); const [languageHints, setLanguageHints] = useState(""); const [keyterms, setKeyterms] = useState(""); const [liveCaptions, setLiveCaptions] = useState(false);
  const [meetingContext, setMeetingContext] = useState<MeetingContextDraft>({ clientName: "", objective: "", clientContext: "" });
  const [calendarContexts, setCalendarContexts] = useState<Record<string, MeetingContextDraft>>({});
  ...
  const updateMeetingContext = (patch: Partial<MeetingContextDraft>) => setMeetingContext((current) => ({ ...current, ...patch }));
  const prepareMeetingContext = async (draft: MeetingContextDraft, key: string) => {
    setBusy(key); setError(undefined); setNotice(undefined);
    try {
      const prepared = await chuskyApi.meetings.prepare({ clientName: draft.clientName.trim(), ...(draft.objective.trim() ? { objective: draft.objective.trim() } : {}), ...(draft.clientContext.trim() ? { clientContext: draft.clientContext.trim() } : {}) });
      const next = { ...draft, brief: prepared.brief, preparedAt: prepared.preparedAt };
      if (key === "prepare-context") setMeetingContext(next);
      else setCalendarContexts((current) => ({ ...current, [key]: next }));
      setNotice("Private meeting brief prepared. Review it before attaching it to the meeting.");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not prepare meeting context."); }
    finally { setBusy(undefined); }
  };
  const calendarDraft = (item: MeetingWorkspace["preparations"][number]): MeetingContextDraft => calendarContexts[item.id] ?? { clientName: item.participants[0] || item.title || "", objective: item.title ? `Prepare for ${item.title}` : "", clientContext: "" };
  const updateCalendarContext = (item: MeetingWorkspace["preparations"][number], patch: Partial<MeetingContextDraft>) => setCalendarContexts((current) => ({ ...current, [item.id]: { ...calendarDraft(item), ...patch } }));
  ...
  const joinMeeting = async () => {
    ...
    try {
      const meeting = await chuskyApi.meetings.join({ meetingUrl: meetingUrl.trim(), interactionMode: effectiveJoinMode, languageMode, liveCaptions, ...(hints.length ? { languageHints: hints } : {}), ...(terms.length ? { keyterms: terms } : {}), ...(selectedRoomId ? { roomId: selectedRoomId } : {}), ...(meetingContext.brief ? { clientName: meetingContext.clientName.trim(), ...(meetingContext.objective.trim() ? { objective: meetingContext.objective.trim() } : {}), ...(meetingContext.clientContext.trim() ? { clientContext: meetingContext.clientContext.trim() } : {}) } : {}) });
      setWorkspace((current) => current ? { ...current, meetings: [meeting, ...current.meetings.filter((item) => item.id !== meeting.id)] } : current); setMeetingUrl(""); setLanguageHints(""); setKeyterms(""); setLiveCaptions(false); setMeetingContext({ clientName: "", objective: "", clientContext: "" }); setNotice(meetingContext.brief ? "Chusky is joining with your reviewed private brief attached." : "Chusky is joining the meeting without private relationship context. The session will appear below as its provider status changes.");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not join this meeting."); }
    finally { setBusy(undefined); }
  };
  const joinPreparedMeeting = async (item: MeetingWorkspace["preparations"][number]) => {
    const draft = calendarDraft(item);
    const attachContext = Boolean(draft.brief);
    if (attachContext && !draft.clientName.trim()) return;
    setBusy(`join-${item.id}`); setError(undefined); setNotice(undefined);
    try {
      const result = await chuskyApi.meetings.joinPrepared(item.id, attachContext ? { clientName: draft.clientName.trim(), ...(draft.objective.trim() ? { objective: draft.objective.trim() } : {}), ...(draft.clientContext.trim() ? { clientContext: draft.clientContext.trim() } : {}) } : {});
      await load(false);
      setNotice(attachContext ? "Chusky is joining with your reviewed private brief attached." : "Chusky is joining the prepared meeting without private relationship context.");
      if (result.id) setCalendarContexts((current) => ({ ...current, [item.id]: draft }));
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not join the prepared meeting."); }
    finally { setBusy(undefined); }
  };
  const joinPreparedWithoutContext = async (item: MeetingWorkspace["preparations"][number]) => {
    setBusy(`join-${item.id}`); setError(undefined); setNotice(undefined);
    try {
      await chuskyApi.meetings.joinPrepared(item.id);
      await load(false);
      setNotice("Chusky is joining the prepared meeting without private relationship context.");
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not join the prepared meeting."); }
    finally { setBusy(undefined); }
  };
  ...
  return <>
    <PageHeading eyebrow="Calendar & live sessions" title="Meetings" description="Review calendar meeting preparations, participant context, live-call outcomes, and the representative profile Chusky uses. Calendar triggers can also describe non-meeting events; only verified supported video links become meeting preparations." action={<div className="flex items-center gap-2"><label className="flex items-center gap-1.5 text-[11px] text-muted-foreground"><input type="checkbox" checked={liveCaptions} onChange={(event) => setLiveCaptions(event.target.checked)}/> Captions</label><Button secondary onClick={() => void load()}><span className="hidden sm:inline-flex"><RefreshCw size={13}/></span> Refresh</Button></div>} />
    ...
    <section className="mb-5"><div className="mb-2 flex items-end justify-between gap-3"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Calendar briefings</p><h2 className="mt-1 font-display text-2xl">Prepared events</h2></div><span className="text-[10px] text-muted-foreground">Refreshes every 15 seconds</span></div>
      {!workspace ? <Card className="flex items-center gap-2 p-4 text-xs text-muted-foreground"><LoaderCircle size={14} className="animate-spin"/> Loading meeting data…</Card> : workspace.preparations.length ? <div className="grid gap-3">{workspace.preparations.map((item) => { const draft = calendarDraft(item); const canJoin = (item.status === "prepared" || item.status === "auto_scheduled") && item.meetingUrlAvailable; return <Card key={item.id} className="p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><CalendarClock size={15} className="text-muted-foreground"/><h3 className="break-words text-sm font-medium">{item.title || "Calendar event"}</h3><Status tone={item.status === "cancelled" || item.status === "expired" ? "gray" : item.status === "prepared" || item.status === "auto_scheduled" ? "green" : "amber"}>{item.status.replaceAll("_", " ")}</Status></div><p className="mt-1.5 text-[11px] text-muted-foreground">{date(item.startAt)}{item.endAt ? ` – ${date(item.endAt)}` : ""} · {item.lifecycle.replaceAll("_", " ")}</p></div>{canJoin && <div className="flex flex-wrap gap-2">{draft.brief && <Button secondary disabled={busy === `join-${item.id}`} onClick={() => void joinPreparedMeeting(item)}>{busy === `join-${item.id}` ? <LoaderCircle size={13} className="animate-spin"/> : <ShieldCheck size={13}/>} Join with brief</Button>}<Button disabled={busy === `join-${item.id}`} onClick={() => void joinPreparedWithoutContext(item)}>Join without context</Button></div>}</div>
        ...
        <div className="mt-3 border-t border-foreground/10 pt-3"><p className="font-mono text-[9px] uppercase tracking-[0.15em] text-muted-foreground">Calendar event brief</p>{item.brief ? <p className="mt-1.5 whitespace-pre-wrap text-xs leading-5">{item.brief}</p> : <p className="mt-1.5 text-[11px] text-muted-foreground">{item.briefStatus === "processing" || item.briefStatus === "queued" ? "Brief is being prepared…" : item.status === "cancelled" ? "This event was cancelled; it is not a joinable meeting." : item.meetingUrlAvailable ? "No event recommendation is available yet. You can prepare a private relationship brief below before joining." : "No supported meeting link was found. This calendar event is not treated as a meeting."}</p>}{canJoin && <MeetingContextPanel compact value={draft} onChange={(patch) => updateCalendarContext(item, patch)} onPrepare={() => void prepareMeetingContext(draft, item.id)} busy={busy === item.id}/>}</div>
...
function capabilityEntries(capabilities?: MeetingProviderCapabilities) {
  if (!capabilities) return [];
  return ["audio", "outboundChat", "inboundChat", "screenShare", "participantEvents", "speakerEvents", "scheduledJoin", "waitingRoom", "admissionControl", "transcript"].map((key) => ({ key, value: capabilities[key as keyof MeetingProviderCapabilities] as string })).filter((entry) => typeof entry.value === "string");
}

function MeetingCard({ meeting }: { meeting: Meeting }) {
  const metrics = meeting.turnMetrics;
  const escalation = meeting.outcome?.escalation;
  return <Card className="p-4 sm:p-5"><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-medium">{meeting.title || `${meeting.platform} meeting`}</h3><Status tone={meeting.status === "in_call" || meeting.status === "joining" ? "green" : meeting.error ? "amber" : "gray"}>{meeting.status.replaceAll("_", " ")}</Status><Status tone={meeting.runtimeState === "degraded" || meeting.runtimeState === "voice_unavailable" ? "amber" : meeting.runtimeState === "healthy" ? "green" : "gray"}>{runtimeLabel(meeting.runtimeState)}</Status><span className="text-[10px] text-muted-foreground">{meeting.interactionMode} · {date(meeting.joinAt || meeting.createdAt)}</span></div>
    {(meeting.languageMode || meeting.languageHints?.length || meeting.keyterms?.length) && <p className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px] text-muted-foreground"><Languages size={12}/>{meeting.languageMode === "multilingual" ? `Multilingual · ${(meeting.languageHints ?? []).join(", ") || "auto-detect"}` : "English audio"}{meeting.keyterms?.length ? ` · ${meeting.keyterms.length} keyterm${meeting.keyterms.length === 1 ? "" : "s"}` : ""}</p>}
    {meeting.error && <p className="mt-2 text-xs text-amber-800">{meeting.error}</p>}
    {metrics && <div className="mt-3 grid grid-cols-2 gap-2 border-y border-foreground/10 py-3 text-[10px] sm:grid-cols-5"><div><p className="text-muted-foreground">Turns</p><p className="mt-1 font-medium">{metrics.turns}</p></div><div><p className="text-muted-foreground">Completed</p><p className="mt-1 font-medium">{metrics.completedTurns}</p></div><div><p className="text-muted-foreground">Fallbacks</p><p className="mt-1 font-medium">{metrics.fallbackTurns}</p></div><div><p className="text-muted-foreground">First audio</p><p className="mt-1 font-medium">{metrics.firstAudio?.p50Ms ? `${Math.round(metrics.firstAudio.p50Ms)} ms p50` : "Not recorded"}</p></div><div><p className="text-muted-foreground">Response</p><p className="mt-1 font-medium">{metrics.finalResponse?.p95Ms ? `${Math.round(metrics.finalResponse.p95Ms)} ms p95` : "Not recorded"}</p></div></div>}
    {meeting.participantRoster.length > 0 && <div className="mt-3"><p className="flex items-center gap-1.5 font-mono text-[9px] uppercase tracking-[0.15em] text-muted-foreground"><Users size={12}/> Participants</p><p className="mt-1.5 text-[11px]">{meeting.participantRoster.map((item) => `${item.name}${item.isHost ? " (host)" : ""}${item.status === "left" ? " · left" : ""}`).join(" · ")}</p></div>}
    {meeting.capabilities && <details className="mt-3 border-t border-foreground/10 pt-3"><summary className="cursor-pointer list-none text-[10px] font-medium"><span className="inline-flex items-center gap-1.5"><ShieldCheck size={12}/> Provider capability matrix</span></summary><div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1.5 text-[10px] sm:grid-cols-3">{capabilityEntries(meeting.capabilities).map(({ key, value }) => <div key={key} className="flex items-center justify-between gap-2"><span className="text-muted-foreground">{capabilityLabel(key)}</span><span className={capabilityTone(value)}>{value}</span></div>)}</div>{meeting.capabilities.notes.length > 0 && <p className="mt-2 text-[10px] leading-4 text-muted-foreground">{meeting.capabilities.notes.join(" ")}</p>}</details>}
    {meeting.timeline?.length ? <details className="mt-3 border-t border-foreground/10 pt-3"><summary className="cursor-pointer list-none text-[10px] font-medium"><span className="inline-flex items-center gap-1.5"><Activity size={12}/> Runtime timeline · {meeting.timeline.length} events</span></summary><div className="mt-2 space-y-2">{meeting.timeline.slice(-10).reverse().map((event) => <div key={event.id} className="flex gap-2 text-[10px]"><Clock3 size={12} className="mt-0.5 shrink-0 text-muted-foreground"/><span className="min-w-24 text-muted-foreground">{date(event.at)}</span><span className="min-w-24 font-mono text-[9px] uppercase tracking-[0.08em] text-muted-foreground">{timelineLabel(event.type)}</span><span>{event.summary}</span></div>)}</div></details> : null}
    {meeting.history.length > 0 && <div className="mt-3 max-h-48 space-y-2 overflow-y-auto border-t border-foreground/10 pt-3">{meeting.history.slice(-6).map((item, index) => <p key={`${item.createdAt ?? index}-${index}`} className="text-[11px] leading-5"><span className="font-medium">{item.role === "assistant" ? "Chusky" : "Meeting context"}: </span>{item.content}</p>)}</div>}
    {meeting.outcome && <div className="mt-3 border-t border-foreground/10 pt-3"><p className="text-xs font-medium">Outcome · {meeting.outcome.title}</p><p className="mt-1 text-[11px] leading-5 text-muted-foreground">{meeting.outcome.summary}</p>{meeting.outcome.decisions.length > 0 && <p className="mt-2 text-[11px]">Decisions: {meeting.outcome.decisions.join(" · ")}</p>}{meeting.outcome.actionItems.length > 0 && <ul className="mt-2 list-disc space-y-1 pl-4 text-[11px] text-muted-foreground">{meeting.outcome.actionItems.map((item, index) => <li key={`${item.task}-${index}`}>{item.task} — {item.owner}{item.dueDate ? ` · ${item.dueDate}` : ""}</li>)}</ul>}{escalation?.required && <div className="mt-3 border border-amber-300 bg-amber-50 p-3 text-[11px] text-amber-950"><p className="font-medium">Owner review after meeting · {escalation.severity}</p>{escalation.reason && <p className="mt-1 leading-4">{escalation.reason}</p>}{escalation.nextSteps.length > 0 && <ul className="mt-2 list-disc space-y-1 pl-4">{escalation.nextSteps.map((step, index) => <li key={`${step.action}-${index}`}>{step.action}{step.owner ? ` — ${step.owner}` : ""}{step.dueDate ? ` · ${step.dueDate}` : ""}</li>)}</ul>}{escalation.openQuestions.length > 0 && <p className="mt-2">Open questions: {escalation.openQuestions.join(" · ")}</p>}<p className="mt-2 text-[10px]">This review is intentionally delivered after the meeting; Chusky does not escalate live conversations directly.</p></div>}<p className="mt-2 text-[10px] text-muted-foreground">Follow-through: {meeting.outcomeStatus || "pending"}{meeting.outcomeNotificationStatus ? ` · notification ${meeting.outcomeNotificationStatus}` : ""}</p></div>}</Card>;
}
