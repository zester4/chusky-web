    ...
    <ProductPageShell>
      ...
      <section id="quickstart" className="border-b border-foreground/10 py-14 sm:py-20 lg:py-28">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-12">
          <div><p className="font-mono text-xs uppercase tracking-[0.18em] text-muted-foreground">01 · Quickstart</p><h2 className="mt-4 font-display text-3xl tracking-tight sm:text-5xl">A useful agent in minutes.</h2><p className="mt-5 text-sm leading-relaxed text-muted-foreground">The SDK is designed for trusted server environments. Your application owns authentication; Chusky owns agent execution, tools, and durable state.</p></div>
      ...
      <section id="runs" className="border-b border-foreground/10 py-14 sm:py-20 lg:py-28">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-12"><div className="max-w-3xl"><p className="font-mono text-xs uppercase tracking-[0.18em] text-muted-foreground">02 · The execution model</p><h2 className="mt-4 font-display text-3xl tracking-tight sm:text-5xl">Conversation is the interface. Runs are the engine.</h2><p className="mt-5 text-sm leading-relaxed text-muted-foreground sm:text-base">A run can stream text, call connected tools, request human approval, create a task, and be inspected after a network interruption. Background continuation depends on the required production services being configured.</p></div><div className="mt-10 grid gap-3 sm:grid-cols-3"><div className="border border-foreground/10 p-5"><BookOpen className="h-5 w-5" /><h3 className="mt-8 font-display text-2xl">Threads</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Threads are owner-scoped. Cross-channel history is not automatic: linked private work and shared channel conversations follow different context rules.</p></div><div className="border border-foreground/10 p-5"><ShieldCheck className="h-5 w-5" /><h3 className="mt-8 font-display text-2xl">Approvals</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Approval policy can pause consequential actions. A decision is exact, owner-scoped, and time-limited.</p></div><div className="border border-foreground/10 p-5"><Braces className="h-5 w-5" /><h3 className="mt-8 font-display text-2xl">Events</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Typed run events expose progress, tool activity, failures, and completion to your interface.</p></div></div></div>
      </section>

      <section id="files" className="border-b border-foreground/10 py-14 sm:py-20 lg:py-28">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:px-12"><div><p className="font-mono text-xs uppercase tracking-[0.18em] text-muted-foreground">03 · Files</p><h2 className="mt-4 font-display text-3xl tracking-tight sm:text-5xl">Give the agent verified context.</h2><p className="mt-5 text-sm leading-relaxed text-muted-foreground">Files are uploaded through short-lived, secure links. Chusky verifies ownership, size, and content type before a file reaches the model.</p></div><div className="space-y-3">{["Create an upload intent", "Upload to a secure link", "Complete server-side verification", "Attach the verified file ID to a run"].map((item) => <div key={item} className="flex items-center gap-3 border border-foreground/10 px-4 py-4 text-sm"><Check className="h-4 w-4 text-emerald-600" />{item}</div>)}</div></div>
      </section>

      <section id="durable" className="py-14 sm:py-20 lg:py-28"><div className="mx-auto flex max-w-6xl flex-col justify-between gap-8 px-4 sm:px-6 lg:flex-row lg:items-end lg:px-12"><div className="max-w-3xl"><p className="font-mono text-xs uppercase tracking-[0.18em] text-muted-foreground">04 · Keep building</p><h2 className="mt-4 font-display text-3xl tracking-tight sm:text-5xl">Tasks, webhooks, models, and more.</h2><p className="mt-5 text-sm leading-relaxed text-muted-foreground">Use the complete SDK documentation for model selection, file workflows, approvals, durable tasks, webhooks, security, errors, and production operations.</p></div><Link href="https://github.com/zester4/chusky-sdk/tree/main/docs" className="inline-flex items-center gap-2 text-sm font-medium underline underline-offset-4">Open full SDK docs <ArrowRight className="h-4 w-4" /></Link></div></section>
    </ProductPageShell>
  );
}
